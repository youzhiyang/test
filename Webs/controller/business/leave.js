(function (parent, leave) {
    /**
     * @description 正在执行操作
     */
    var _doing = false;

    /**
    * 引导人数据源
    */
    var _leavveGuideDataSource = [];

    /**
     * @description 数据源
     */
    var _dataSource = [];

    /**
     * @description 添加
     */
    var _addArr = [];

    /**
     * @description 未登记
     */
    var _notRegister = [];

    /**
     * @description 页面加载
     */
    $(document).ready(function () {
        initDataSource();
        initUI();
    });

    /**
     * @description 读取身份证件
     * @param {Object} cert 身份证件信息 
     */
    leave.readCard = function (cert) {
        if (_doing === true) {
            return;
        }
        var myCert = {
            VisitOrgization: "",
            Phone: "",
            CertID: cert.certNumber,
            CertTypeCode: "IdentityCard",
            CertTypeName: "身份证",
            Name: cert.partyName,
            Gender: cert.gender,
            Nation: cert.nation,
            BornDay: cert.bornDay,
            CertAddress: cert.certAddress,
            CertOrg: cert.certOrg,
            EffDate: cert.effDate,
            ExpDate: cert.expDate,
            IdentityPic: cert.identityPic,
            PicFront: cert.base64ID_PicFront,
            PicBack: cert.base64ID_PicBack
        };
        if (myCert.CertID.isNullOrEmpty()) {
            return;
        }
        for (var i = 0; i < _dataSource._data.length; i++) {
            if (_dataSource._data[i].CertID === myCert.CertID) {
                return;
            }
        }
        getVisitorRecord(myCert);
        /*$("#leaveCertNation").val(myCert.Nation);
        $("#leaveCertBornDay").val(myCert.BornDay);
        $("#leaveCertCertAddress").val(myCert.CertAddress);
        $("#leaveCertCertOrg").val(myCert.CertOrg);
        $("#leaveCertEffDate").val(myCert.EffDate);
        $("#leaveCertExpDate").val(myCert.ExpDate);*/
    };

    /********************* DataSource begin **********************/
    function initDataSource() {
        _dataSource = new kendo.data.DataSource({
            schema: {
                model: {
                    fields: {
                        VisitOrgization: { type: "string" },
                        Phone: { type: "string" },
                        CertID: { type: "string" },
                        CertTypeCode: { type: "string" },
                        CertTypeName: { type: "string" },
                        Name: { type: "string" },
                        Gender: { type: "string" },
                        Nation: { editable: false, type: "string" },
                        BornDay: { editable: false, type: "date" },
                        CertAddress: { editable: false, type: "string" },
                        CertOrg: { editable: false, type: "string" },
                        EffDate: { editable: false, type: "date" },
                        ExpDate: { editable: false, type: "date" }
                    },
                    id: "CertID"
                }
            },
            serverPaging: false,
            serverSorting: false,
            serverFiltering: false
        });
        _leavveGuideDataSource = new kendo.data.DataSource({
            error: function (e) {
                layer.open({
                    btn: "确定",
                    content: e.xhr.responseText,
                    icon: 2,
                    yes: function (index) {
                        //权限错误
                        if (e.xhr.statusText === "Unauthorized") {
                            window.location.replace("index.html");
                        } else {
                            layer.close(index);
                        }
                    }
                });
            },
            schema: {
                data: function (response) {
                    if (response !== null) {
                        if (response.errcode === 0) {
                            return JSON.parse(response.DataJSON);
                        } else {
                            layer.open({
                                btn: "确定",
                                content: response.errmsg,
                                icon: 2
                            });
                            return [];
                        }
                    }
                },
                model: {
                    fields: {
                        StaffID: { type: "int" },
                        StaffName: { type: "string" },
                        StaffJob: { type: "string" },
                        StaffSex: { type: "string" },
                        StaffHireDate: { type: "date" },
                        StaffAddr: { type: "string" },
                        StaffCompany: { type: "string" },
                        StaffNumber: { type: "string" }
                    },
                    id: "StaffID"
                }
            },
            transport: {
                read: {
                    contentType: "application/json; charset=utf-8",
                    crossDomain: true,
                    dataType: "json",
                    type: "get",
                    url: system.api.business.enter.getStaff,
                    xhrFields: {
                        withCredentials: true
                    }
                }
            },
            serverPaging: false,
            serverSorting: false,
            serverFiltering: false
        });
    }
    /********************* DataSource end **********************/

    /********************* UI begin **********************/

    /**
     * @description 初始化UI
     */
    function initUI() {
        /*var myCert = {
            VisitOrgization: "",
            Phone: "",
            CertID: "360124199402274514",
            CertTypeCode: "IdentityCard",
            CertTypeName: "身份证",
            Name: "游志洋",
            Gender: "男",
            Nation: "汉",
            BornDay: "1994-02-27",
            CertAddress: "江西南昌",
            CertOrg: "江西南昌",
            EffDate: "2017-01-01",
            ExpDate: "2027-01-01",
            IdentityPic: "",
            PicFront: "",
            PicBack: ""
        };
        getVisitorRecord(myCert);
        var myCert1 = {
            VisitOrgization: "",
            Phone: "",
            CertID: "360124199402274516",
            CertTypeCode: "IdentityCard",
            CertTypeName: "身份证",
            Name: "游志洋",
            Gender: "男",
            Nation: "汉",
            BornDay: "1994-02-27",
            CertAddress: "江西南昌",
            CertOrg: "江西南昌",
            EffDate: "2017-01-01",
            ExpDate: "2027-01-01",
            IdentityPic: "",
            PicFront: "",
            PicBack: ""
        };
        setTimeout(function () {
            getVisitorRecord(myCert1);
        }, 3000);*/
        kendo.culture("zh-CN");
        $(".leave_cert_reader input").each(function () {
            $(this).kendoTextBox({
                readonly: true
            })
        });
        $("#leaveGuidePerson").kendoDropDownList({
            autoBind: false,
            dataSource: _leavveGuideDataSource,
            dataTextField: "StaffName",
            dataValueField: "StaffID",
            optionLabel: "",
            noDataTemplate: "没有数据",
            text: "请选择引导人",
            index: -1
        });
        $("#leaveVistor").kendoGrid({
            batch: true,
            columns: [
                {
                    attributes: { class: "center_column", title: "#: VisitOrgization === null ? '' : VisitOrgization #" },
                    field: "VisitOrgization",
                    template: "#: VisitOrgization === null ? '' : VisitOrgization #",
                    title: " 来访单位",
                    width: 250,
                    editable: function (dataItem) {
                        return false;
                    }
                }, {
                    attributes: { class: "center_column", title: "#: Phone === null ? '' : Phone #" },
                    field: "Phone",
                    template: "#: Phone === null ? '' : Phone #",
                    title: " 电话号码",
                    width: 200,
                    editable: function (dataItem) {
                        return false;
                    }
                }, {
                    attributes: { class: "center_column", title: "#: CertID === null ? '' : CertID #" },
                    editable: false,
                    field: "CertID",
                    template: "#: CertID === null ? '' : CertID #",
                    title: " 证件号码",
                    width: 250,
                    editable: function (dataItem) {
                        return isEdit(dataItem);
                    }
                }, {
                    attributes: { class: "center_column", title: "#: Name === null ? '' : Name #" },
                    editable: false,
                    field: "Name",
                    template: "#: Name === null ? '' : Name #",
                    title: "姓名",
                    width: 150,
                    editable: function (dataItem) {
                        return false;
                    }
                }, {
                    attributes: { class: "center_column", title: "#: Gender === null ? '' : Gender #" },
                    field: "Gender",
                    template: "#: Gender === null ? '' : Gender #",
                    title: "性别",
                    width: 100,
                    editable: function (dataItem) {
                        return false;
                    }
                }, {
                    attributes: { class: "center_column", title: "#: Name === null ? '' : Name #" },
                    field: "Name",
                    template: function (data) {
                        if (data.IdentityPic.isNullOrEmpty()) {
                            return "<div class=\"photo_container\"></div>";
                        } else {
                            return "<div class=\"photo_container\"><img src=\"data:image/jpeg;base64," + data.IdentityPic + "\" /></div>";
                        }
                    },
                    title: "相片",
                    width: 100,
                    editable: function (dataItem) {
                        return false;
                    }
                }, {
                    command: "destroy",
                    title: "编辑",
                    width: 100
                    //}, {
                    //    attributes: { class: "", title: "#: EffDate === null ? '' : EffDate + ' 至 ' + ExpDate === null ? '' : ExpDate #" },
                    //    field: "CertOrg",
                    //    template: "#: EffDate === null ? '' : EffDate + ' 至 ' + ExpDate === null ? '' : ExpDate #",
                    //    title: "有效期",
                    //    width: 200
                    //}, {
                    //    attributes: { class: "center_column", title: "#: Nation === null ? '' : Nation #" },
                    //    field: "Nation",
                    //    template: "#: Nation === null ? '' : Nation #",
                    //    title: "民族",
                    //    width: 100
                    //}, {
                    //    attributes: { class: "", title: "#: BornDay === null ? '' : BornDay #" },
                    //    field: "BornDay",
                    //    template: "#: BornDay === null ? '' : BornDay #",
                    //    title: "出生日期",
                    //    width: 120
                    //}, {
                    //    attributes: { class: "right_column", title: "#: CertAddress === null ? '' : CertAddress #" },
                    //    field: "CertAddress",
                    //    template: "#: CertAddress === null ? '' : CertAddress #",
                    //    title: "证件地址",
                    //    width: 150
                    //}, {
                    //    attributes: { class: "", title: "#: CertOrg === null ? '' : CertOrg #" },
                    //    field: "CertOrg",
                    //    template: "#: CertOrg === null ? '' : CertOrg #",
                    //    title: "发证机关",
                    //    width: 200
                }],
            dataSource: _dataSource,
            editable: {
                confirmation: false,
                mode: "incell",
                confirmDelete: "Yes"
            },
            groupable: false,
            navigatable: false,
            resizable: true,
            sortable: false,
            remove: function (e) {
                if (_addArr.indexOf(e.model.uid) > -1) {
                    _addArr.remove(e.model.uid);
                }
                var tempArr = _dataSource._data.filter(function (item) {
                    return item.uid !== e.model.uid;
                });
                if (tempArr.length > 0) {
                    var data = _dataSource._data[_dataSource._data.length - 1];
                    setCertInfo(data.CertID, data.Name, data.Gender);
                } else {
                    setCertInfo();
                }
            },
            cellClose: function (e) {
                var reg = /^\d{6}(18|19|20)\d{2}(0\d|10|11|12)([0-2]\d|30|31)\d{3}(\d|X|x)$/;
                if (reg.test(e.model.CertID)) {
                    getHistoryVisitorRecord(e.model);
                } else {
                    if (!e.model.CertID.isNullOrEmpty()) {
                        layer.open({
                            btn: "确定",
                            content: "请填写正确的证件号码。",
                            icon: 2
                        });
                    }
                    for (var i = 0; i < _dataSource._data.length; i++) {
                        if (e.model.uid === _dataSource._data[i].uid) {
                            e.model.VisitOrgization = "";
                            e.model.Phone = "";
                            e.model.Name = "";
                            e.model.Gender = "";
                            e.model.IdentityPic = "";
                            _dataSource._data[i] = e.model;
                            setCertInfo();
                            refresh();
                        }
                    }
                }
            }
        });
        $("#leaveSubmit").kendoButton();
        $("#leaveClear").kendoButton();
        $("#leaveReturn").kendoButton();
    }

    /********************* UI end **********************/

    /********************* Event begin **********************/

    /**
     * @description 返回
     */
    $("#leaveReturn").on("click", function (e) {
        window.register.return();
        window.register.startScroll();
    });

    /**
     * @description 清空
     */
    $("#leaveClear").on("click", function (e) {
        leave.clearForm();
    });

    /**
     * @description 提交
     */
    $("#leaveSubmit").on("click", function (e) {
        submitLeave();
    });

    /**
     * @description 添加
     */
    $("#leaveAdd").on("click", function (e) {
        leaveAdd();
    });

    /********************* Event end **********************/

    /********************* Business begin **********************/

    /**
     * @description 是否可编辑
     */
    function isEdit(dataItem) {
        if (_addArr.indexOf(dataItem.uid) > -1) {
            return true;
        }
        return false;
    }

    /**
    * @description 刷新
    */
    function refresh() {
        var grid = $("#leaveVistor").data("kendoGrid");
        grid.refresh();
    }

    /**
     * @description 根据身份证id获取访客信息(刷卡)
     */
    function getVisitorRecord(myCert) {
        var data = {
            CertID: myCert.CertID
        }
        window.system.ajax.post(window.system.api.business.leave.getVisitorRecord, data, function (data, status, xhr) {
            if (data.errcode === 0) {
                setCertInfo(myCert.CertID, myCert.Name, myCert.Gender);
                var visitorRecord = JSON.parse(data.DataJSON);
                myCert.VisitOrgization = visitorRecord.VisitOrgization;
                myCert.Phone = visitorRecord.Phone;
                _dataSource._data.push(myCert);
            } else {
                layer.open({
                    btn: "确定",
                    content: data.errmsg,
                    icon: 2
                });
            }
        })
    }

    /**
     * @description 根据身份证id获取访客信息(新增)
     */
    function getHistoryVisitorRecord(myCert) {
        var data = {
            CertID: myCert.CertID
        }
        window.system.ajax.post(window.system.api.business.leave.getVisitorRecord, data, function (data, status, xhr) {
            if (data.errcode === 0) {
                var visitorRecord = JSON.parse(data.DataJSON);
                myCert.VisitOrgization = visitorRecord.VisitOrgization;
                myCert.Phone = visitorRecord.Phone;
                myCert.Name = visitorRecord.Name;
                myCert.Gender = visitorRecord.Gender;
                myCert.IdentityPic = visitorRecord.IdentityPic;
                for (var i = 0; i < _dataSource._data.length;i++) {
                    if (myCert.uid === _dataSource._data[i].uid) {
                        _dataSource._data[i] = myCert;
                        _notRegister.remove(myCert.uid);
                        setCertInfo(myCert.CertID, myCert.Name, myCert.Gender);
                        refresh();
                    }
                }
            } else {
                myCert.VisitOrgization = "";
                myCert.Phone = "";
                myCert.Name = "";
                myCert.Gender = "";
                myCert.IdentityPic = "";
                for (var i = 0; i < _dataSource._data.length; i++) {
                    if (myCert.uid === _dataSource._data[i].uid) {
                        _dataSource._data[i] = myCert;
                        setCertInfo();
                        refresh();
                    }
                }
                if (data.errcode == -2) {
                    _notRegister.push(myCert.uid);
                }
                layer.open({
                    btn: "确定",
                    content: data.errmsg,
                    icon: 2
                });
            }
        })
    }

    /**
     * @description 提交离场信息
     */
    function submitLeave() {
        if (_dataSource._data.length === 0) {
            layer.open({
                btn: "确定",
                content: "请扫描身份证。",
                icon: 2
            });
            return;
        }
        if ($("#leaveGuidePerson").data("kendoDropDownList").value().isNullOrEmpty()) {
            layer.open({
                btn: "确定",
                content: "请选择引导人。",
                icon: 2
            });
            return;
        };
        for (var i = 0; i < _dataSource._data.length; i++) {
            if (_dataSource._data[i].CertID.isNullOrEmpty()) {
                layer.open({
                    btn: "确定",
                    content: "请填写证件号码。",
                    icon: 2
                });
                return;
            } else {
                var reg = /^\d{6}(18|19|20)\d{2}(0\d|10|11|12)([0-2]\d|30|31)\d{3}(\d|X|x)$/;
                if (!reg.test(_dataSource._data[i].CertID.trim())) {
                    layer.open({
                        btn: "确定",
                        content: "请填写正确的证件号码。",
                        icon: 2
                    });
                    return;
                }
                if (_notRegister.indexOf(_dataSource._data[i].uid) > -1) {
                    var errormsg = "证件号码" + _dataSource._data[i].CertID + "未登记";
                    layer.open({
                        btn: "确定",
                        content: errormsg,
                        icon: 2
                    });
                    return;
                }
            }
        }
        var certIds = _dataSource._data.map((item) => {
            return item.CertID;
        });
        if (new Set(certIds).size != certIds.length) {
            layer.open({
                btn: "确定",
                content: "证件号码重复。",
                icon: 2
            });
            return;
        }
        submit();
    }

    /**
     * @description 提交
     */
    function submit() {
        var data = {
            LeaveGuide: $("#leaveGuidePerson").val(),
            VisitorList: _dataSource._data
        };
        _doing = true;
        $("#dialogMask").show();
        system.ajax.post(system.api.business.leave.upsertLeaves, data, function (data, status, xhr) {
            leave.clearForm();
            if (data.errcode === 0) {
                leave.clearForm();
                layer.open({
                    btn: "确定",
                    content: data.errmsg,
                    icon: 1
                });
            } else {
                layer.open({
                    btn: "确定",
                    content: data.errmsg,
                    icon: 2
                });
            }
        }, function (xhr, textStatus, errorThrown) {
            layer.open({
                btn: "确定",
                content: xhr.responseText,
                icon: 2,
                yes: function (index) {
                    layer.close(index);
                }
            });
        }, function () {
            _doing = false;
            $("#dialogMask").hide();
        });
    }

    /**
     * @description 设置身份信息
     */
    function setCertInfo(certId, certName, gender) {
        $("#leaveCertCertID").val(certId);
        $("#leaveCertName").val(certName);
        $("#leaveCertGender").val(gender);
    }

    /**
     * @description 添加
     */
    function leaveAdd() {
        var myCert = {
            VisitOrgization: "",
            Phone: "",
            CertID: "",
            CertTypeCode: "IdentityCard",
            CertTypeName: "身份证",
            Name: "",
            Gender: "",
            Nation: "",
            BornDay: "",
            CertAddress: "",
            CertOrg: "",
            EffDate: "",
            ExpDate: "",
            IdentityPic: "",
            PicFront: "",
            PicBack: ""
        };
        _dataSource._data.push(myCert);
        _addArr.push(_dataSource._data[_dataSource._data.length - 1].uid);
        setCertInfo();
    }

    /**
     * @description 清空表单
     */
    leave.clearForm = function clearForm() {
        setCertInfo();
        $("#leaveGuidePerson").data("kendoDropDownList").index = -1;
        $("#leaveGuidePerson").data("kendoDropDownList").text("请选择引导人");
        _dataSource._data = [];
        _dataSource.read();
        _addArr = [];
        _notRegister = [];
    }

    /********************* Business end **********************/
}(window.business = window.business || {}, window.business.leave = {}));