...
(function (common) {
     ...
     * @param {String} key Cookie的唯一键值
     * @param {String} value Cookie值
     * @param {Number} expire 有效期（分钟）
     * @param {String} domain 域
     */
    common.setCookie = function (key, value, expire, domain) {
        var minutes = expire;
        var exp = new Date();
        ...
        if (expire != 0) {
            ...
            var str = key + "=" + encodeURIComponent(value) + ";path=/;";
        }
     ...
     * @description 尝试转换整数
     * @param {String} value 需验证的值
     * @param {String} replaceVal 不满足时替换的值
     */
    common.tryParseInt = function (value, replaceVal) {
     ...
     * @param {String} replaceVal 不满足时替换的值
     */
    common.tryParseFloat = function (value, replaceVal) {
    ...
    /**
     * @description 获取浏览器版本
     * @return {Object} 浏览器版本信息
     */
    common.browserVersion = function () {
    ...
    /**
     * @description 将对象属性值为空的属性设置为空字符串
     * @param {Object} obj 需操作的对象
     * @return {Object} 操作后的对象
     */
    common.fillNullProperty = function (obj) {
        for (var p in obj) {
            if (obj[p] === null) {
                obj[p] = "";
            }
            else if (obj[p].constructor === Function) {
                continue;
            }
            else if (obj[p].constructor === Object) {
                obj[p] = common.fillNullProperty(obj[p]);
            }
            else if (obj[p].constructor === Array) {
                for (var i = 0; i < obj[p].length; i++) {
                    obj[p][i] = common.fillNullProperty(obj[p][i]);
                }
            }
            else {
                if (!obj[p] || (obj[p] === null)) {
    ...
    /**
     * @description 将数组中对象属性值为空的属性设置为空字符串
     * @param {Array} objects 操作的对象数组
     * @return {Array} 操作后的对象数组
     */
    common.fillNullPropertyInArray = function (objects) {
        var obj = [];
        for (var i = 0; i < objects.length; i++) {
            obj.push(common.fillNullProperty(objects[i]));
        }
        return obj;
    };

    /**
     * @description 将【/Date(1333245600000+0800)/】按【2012-04-01 10:00:00】的格式返回日期
     * @param {RegExp} val 日期
     * @param {String} format 格式
     * @return {String} 格式化后的日期
     */
    common.formatDate = function (val, format) {
        var re = /-?\d+/;
        var m = re.exec(val);
     ...
     * @return {String} 参数值
     */
    common.getUrlStringParam = function (url, name) {
        url = url.substr((url.indexOf("?") >= 0) ? (url.indexOf("?") + 1) : 0);
        var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)");
        var r = url.match(reg);
        if (r !== null) {
            return unescape(r[2]);
    ...
    /**
     * @description 等待计数器满足条件，执行回调函数。
     * @param {Object{value, count, time}} counter 计数器{当前值，目标值，最大等待时间（×0.1秒）}
     * @param {Function} callback 回调函数
     * @param {Object} params 参数
     */
    common.waiting = function (counter, callback, params) {
        if ((counter.value === null) || (counter.value === undefined)) {
            counter.value = 0;
        }
        if ((counter.time === null) || (counter.time === undefined)) {
            counter.time = 150;
        }
        if ((counter.value !== counter.count) && (counter.time-- > 0)) {
            setTimeout(function () { common.waiting(counter, callback, params); }, 100);
        } else {
            if (callback) {
                callback(params);
            }
     ...
     * @description 加载子页面
     */
    common.loadPage = function (url, content) {
        $(content).append(
            $.ajax({
    ...
    (function (math) {
        ...
        math.plus = function (num1, num2) {
            ...
            //取得小数位数
            var floatLength1 = (num1.toString().indexOf(".") >= 0) ? (num1.toString().length - num1.toString().indexOf(".") - 1) : 0;
            var floatLength2 = (num2.toString().indexOf(".") >= 0) ? (num2.toString().length - num2.toString().indexOf(".") - 1) : 0;
            var floatLength = Math.max(floatLength1, floatLength2);
            //补零
            var prePow10 = Math.pow(10, floatLength);
            var preZeros = prePow10.toString().replace("1", "");
            var tailPow10 = Math.pow(10, Math.abs(floatLength1 - floatLength2));
            var tailZeros = tailPow10.toString().replace("1", "");
            //转换为整数
            num1 = parseInt(num1.toString().replace(".", "") + (floatLength1 > floatLength2 ? "" : tailZeros));
            num2 = parseInt(num2.toString().replace(".", "") + (floatLength1 > floatLength2 ? tailZeros : ""));
            var result = num1 + num2;
            var isNegative = result < 0;
            result = preZeros + Math.abs(result).toString();
            //转换为小数
            result = (isNegative ? "-" : "") + result.substr(0, result.length - floatLength)
                + (result.substr(result.length - floatLength).isNullOrEmpty() ? "" : ("." + result.substr(result.length - floatLength)));
        ...
        math.multiply = function (num1, num2) {
            ...
            var floatLength1 = (num1.toString().indexOf(".") >= 0) ? (num1.toString().length - num1.toString().indexOf(".") - 1) : 0;
            var floatLength2 = (num2.toString().indexOf(".") >= 0) ? (num2.toString().length - num2.toString().indexOf(".") - 1) : 0;
            var floatLength = Math.max(floatLength1, floatLength2) * 2;
            //补零
            var pow10Head = Math.pow(10, floatLength);
            var zerosHead = pow10Head.toString().replace("1", "");
            var pow10End = Math.pow(10, Math.abs(floatLength1 - floatLength2));
            var zerosEnd = pow10End.toString().replace("1", "");
            //转换为整数
            num1 = parseInt(num1.toString().replace(".", "") + (floatLength1 > floatLength2 ? "" : zerosEnd));
            num2 = parseInt(num2.toString().replace(".", "") + (floatLength1 > floatLength2 ? zerosEnd : ""));
            var result = zerosHead + (num1 * num2).toString();
            //转回小数
            result = result.substr(0, result.length - floatLength)
                + (result.substr(result.length - floatLength).isNullOrEmpty() ? "" : ("." + result.substr(result.length - floatLength)));
            return parseFloat(isNegative ? -result : result);
        };
        ...
        math.divide = function (num1, num2) {
            ...
            var floatLength1 = (num1.toString().indexOf(".") >= 0) ? (num1.toString().length - num1.toString().indexOf(".") - 1) : 0;
            var floatLength2 = (num2.toString().indexOf(".") >= 0) ? (num2.toString().length - num2.toString().indexOf(".") - 1) : 0;
            var floatLength = floatLength1 - floatLength2 + 2;
            //补零
            var pow10 = Math.pow(10, Math.abs(floatLength1 - floatLength2));
            var zeros = pow10.toString().replace("1", "");
            //被除数必须大于除数
            var pow10Ext = Math.pow(10, Math.abs(floatLength));
            var zerosExt = pow10Ext.toString().replace("1", "");
            //转换为整数
            num1 = parseInt(num1.toString().replace(".", "") + (floatLength1 > floatLength2 ? "" : zeros) + zerosExt);
            num2 = parseInt(num2.toString().replace(".", "") + (floatLength1 > floatLength2 ? zeros : ""));
            var result = zerosExt + parseInt(num1 / num2).toString();
            //转回小数，截取原精度增加2位
            result = result.substr(0, result.length - floatLength)
                + (result.substr(result.length - floatLength).isNullOrEmpty() ? "" : ("." + result.substr(result.length - floatLength)));
            return parseFloat(isNegative ? -result : result);
        ...
        math.average = function (array) {
            return this.divide(sum(array), array.length);
        };
    }(window.common.math = {}));
    ...
    (function (ui) {
        ...
        (function (animation) {
            /**
             * @description 透明度
             * @param {Object} el 目标控件
             * @param {Number} alpha 透明度值
             * @param {Number} time 持续时间（毫秒）
             * @param {Number} clear 透明度值
             */
            animation.opacity = function (el, alpha, time, clear) {
                var timer = null;
                if (clear > 0) {
                    timer = clear;
                }
                clearInterval(timer);
                //缺少必要参数
                if ((el === undefined) || (el === null) || (alpha === undefined) || (alpha === null)) {
                    return;
                }
                el = $(el);
                //当前透明度
                var currentAlpha = parseFloat(el.css("opacity"));
                if (parseInt(time) == 0) {
                    time = 1000;
                }
                //计算增量
                var speed = common.math.divide(common.math.minus(currentAlpha, parseFloat(alpha)), time / 10);
                if (speed === 0) {
                    return;
                }
                //计算增量正负
                ...
                timer = setInterval(function () {
                    ...
                    currentAlpha = common.math.plus(currentAlpha, speed);
                    if ((speed >= 0) && (currentAlpha > alpha)) {
                        currentAlpha = alpha;
                    } else if ((speed < 0) && (currentAlpha < alpha)) {
                        currentAlpha = alpha;
                    }
                    el.css("opacity", currentAlpha);
                }, 10);
                return timer;
            };
        }(window.common.ui.animation = {}));
    }(window.common.ui = {}));
}(window.common = {}));
...
/**
 * @description 通过属性名获取对象的值
 * @param {Objcet} obj 对象
 * @param {String} property 属性名
 * @return {Objcet} 属性值
 */
Object.getPropertyValue = function (obj, property) {
    var result = "";
    for (var p in obj) {
        if (p !== property) {
            continue;
...
/**
 * @description 使用默认值替换空对象
 * @param {Objcet} obj 对象
 * @param {Objcet} replace 属性名
 * @return {Objcet} 替换结果
 */
Object.isNull = function (obj, replace) {
...
/**
 * @description 字符串是否相等
 * @param {String} str 参与比较的字符串
 * @return {Number} 返回首次出现不一致的位置，-1则为一致
 */
...
String.prototype.format = function (args) {
    ...
    else if ((args.constructor === String) || (args.constructor === Number)) {
        var reg = new RegExp("({)0(})", "g");
        result = result.replace(reg, args);
    }
    return result;
}
 ...
 * @return {Array} 参数
 */
String.prototype.separateBy = function (separators, lengths) {
    var result = "";
    ...
    if (separators.constructor != Array) {
        separators = [separators];
    }
    if (lengths.constructor != Array) {
        lengths = [lengths];
    }
    ...
    for (var i = 0, s = 0, l = 0; i < this.length; s++ , l++) {
        result += this.substr(i, lengths[l % lengths.length]);
        i += lengths[l % lengths.length];
        if (i < this.length) {
            result += separators[s % separators.length];
        }
    }
    return result;
}
 ...
 * @param {String} str1 参与比较的字符串1
 * @param {String} str2 参与比较的字符串2
 * @return {Number} str1 < str2 返回 -1；str1 > str2 返回 1；str1 = str2 返回 0
 */
String.compare = function (str1, str2) {
    ...
    function getSpell(str) {
        ...
        for (i = 0; i < str.length; i++) {
            if (str.charCodeAt(i) >= 0x4e00) {
                ...
                if (c > -1 && c < 3755) {
                    for (j = qswhSpell.length - 1; j > 0; j = j - 2) {
                        if (qswhSpell[j] <= c) {
                            break;
                        }
                    }
    ...
    var spell1 = getSpell(str1);
    var spell2 = getSpell(str2);
    if (spell1 > spell2) {
        return 1;
    }
...
String.join = function (separator, array) {
    ...
    if (result.lastIndexOf(separator) >= 0) {
        result = result.substr(0, result.lastIndexOf(separator));
    }
    return result;
}
...
/**
 * @description 将数组合并为字符串
 * @param {String} prefix 每个数组元素的前缀
 * @param {String} suffix 每个数组元素的后缀
 * @param {String} separator 分隔符
 * @param {Array} array 操作的数组
 * @return {String} 合并结果
 */
String.joinFormat = function (prefix, suffix, separator, array) {
    var result = "";
    for (var i = 0; i < array.length; i++) {
        result += prefix + array[i] + suffix + separator;
    }
    if (result.lastIndexOf(separator) >= 0) {
        result = result.substr(0, result.lastIndexOf(separator));
    }
...
// Array的prototype会导致foreach遍历对象属性使用部正常
...
/**
 * @description 将数组去重
 * @param {Array} source 需操作的数组
 * @return {Array} 操作结果
 */
Array.distinct = function (source) {
    var array = [];
    for (var i = 0; i < source.length; i++) {
        if (array.indexOf(source[i]) < 0) {
            array.push(source[i]);
        }
    }
    return array;
...
/**
 * @description 获取数组满足条件的对象
 * @param {Array} source 需操作的数组
 * @param {Objcet} value 可选，参与比较的值
 * @return {String} 操作结果
 */
Array.first = function (source, value) {
    for (var i = 0; i < source.length; i++) {
 ...
 * @description 对数组正序排序
 * @param {Array} source 需操作的数组
 * @param {String} property 可选，参与比较的属性名
 * @return {Array} 排序结果
 */
Array.orderBy = function (source, property) {
    var array = [];
    var temp;
    for (var i = 0; i < source.length; i++) {
        array.push(source[i]);
    ...
    for (var i = 0; i < array.length; i++) {
        for (var j = 0; j < (array.length - 1 - i); j++) {
            if (property) {
                if (typeof (array[j][property]) === "string") {
                    //字符串比较
                    if (String.compare(array[j][property], array[j + 1][property]) > 0) {
                        temp = array[j];
                        array[j] = array[j + 1];
                        array[j + 1] = temp;
                    }
 ...
 * @return {Array} 排序结果
 */
Array.orderByDescending = function (source, property) {
    var array = [];
    var temp;
    for (var i = 0; i < source.length; i++) {
        array.push(source[i]);
...
/**
 * @description 获取数组对象指定属性
 * @param {Array} source 需操作的数组
 * @param {String} property 属性名
 * @return {Array} 指定属性组成的数组
 */
Array.select = function (source, property) {
    var array = [];
    for (var i = 0; i < source.length; i++) {
        //p为属性名称，source[i][p]为对应属性的值
        for (var p in source[i]) {
            if (typeof (source[i][p]) === " function ") {
                continue;
            }
            else if (p === property) {
                array.push(source[i][p]);
                break;
            }
        }
    }
...
/**
 * @description 获取满足条件的数组
 * @param {Array} source 需操作的数组
 * @param {String} property 属性名
 * @param {String} operator 运算符
 * @param {} value 比较指标
 * @return {Array} 符合条件的数组元素组成的数组
 */
Array.where = function (source, property, operator, value) {
    var array = [];
    for (var i = 0; i < source.length; i++) {
        //p为属性名称，source[i][p]为对应属性的值
        for (var p in source[i]) {
            if (typeof (source[i][p]) === " function ") {
                ...
                switch (operator) {
                    ...
                    case "notin":
                        ...
                        break;
                    case "like":
                        if (source[i][p].toString().toLowerCase().indexOf(value.toLowerCase()) >= 0) {
                            array.push(source[i]);
                        }
                        break;
                    default:
...
/**
 * @description 日期计算
 * @param {String} part 计算的部分
 * @param {Number} interval 差值
 * @return {Date} 计算后的日期
 */
Date.prototype.add = function (part, interval) {
    var date = this;
    switch (part) {
        case "y": {
        ...
        default: {
            date.setDate(d.getDate() + interval);
            return date;
        }
    }
}
...
Date.prototype.format = function (format, week) {
    var o = {
        ...
        "w": (week && week.constructor && (week.constructor === Function)) ? week(this.getDay()) : ((week && (week.constructor === Array)) ? week[this.getDay()] : this.getDay())//week
    }
    if (/(y+)/.test(format)) {
